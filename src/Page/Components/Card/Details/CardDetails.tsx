...
import { deleteBinhLuan, guiBinhLuan } from "../../../../Api/AuthHasToken/auth";
...
const CardDetails = () => {
  ...
  const handleComment = async (id: number) => {
    ...
    try {
      ...
      setNotificationMessage(
        error.response?.data?.error || error?.error || "Gửi bình luận thất bại."
      );
      setOpenNotification(true);
    } finally {
      setLoading1(false);
  ...
  const handleDeleteComment = async (commentId: number) => {
    try {
      if (!token) {
        setNotificationMessage("Bạn chưa đăng nhập!");
        setOpenNotification(true);
        return;
      }

      await deleteBinhLuan(commentId);

      fetchBinhLuan(id);

      setNotificationMessage("Xoá bình luận thành công!");
      setOpenNotification(true);
    } catch (error: any) {
      setNotificationMessage(
        error.response?.data?.error || "Không thể xoá bình luận!"
      );
      setOpenNotification(true);
    }
  };
  ...
  if (loading)
    return (
      <div className="detail">
        <Skeleton.Input
          active
          style={{ width: 250, height: 32, marginBottom: 20 }}
        />
  ...
  return (
    <div className="detail">
      <div
        className={`view-card ${
          fixedBottom ? "header-fixed-padding-detail" : ""
        }`}
      >
        ...
        />
        <div
          className="view-card-image-wrap"
        ...
        <div>
          <div>
            <div
              className="section-header-tinmoinhat"
              style={{ background: "none", margin: "5px 0px" }}
            >
